    ...
    <section id="tutors" className="section bg-[#fbfcfb]">
      <div className="max-w-310 mx-auto px-4 py-15 lg:py-18">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-14">
          ...
          <a
            className="btn btn-ghost btn-icon-r self-start md:self-auto"
            href="#all-tutors"
          >
            Browse all tutors
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {TUTORS.map((t) => (
            <article className="tutor-card" key={t.name}>
              <div
                className={`relative w-full aspect-square rounded-2.5xl overflow-hidden mb-4 ${t.photo}`}
              >
                <FaceArt kind={t.face} />
                ...
                <button
                  aria-label="Save"
                  className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/95 grid place-items-center text-muted-2 hover:text-rose hover:scale-110 transition"
                >
              ...
              <div className="flex items-start justify-between gap-2 mb-1">
                ...
                <div className="flex items-center gap-1 text-[13px] font-semibold text-ink shrink-0">
                  ...
                  {t.rating}
                  <span className="text-muted font-normal">({t.reviews})</span>
                </div>
              </div>
              ...
              <div className="flex gap-1.5 mb-4 flex-wrap">
                {t.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[11.5px] font-medium text-ink-2 bg-line-2 px-2.5 py-1 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
