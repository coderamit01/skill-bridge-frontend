    ...
    <section id="tutors" className="section bg-[#fbfcfb]">
      <div className="max-w-310 mx-auto px-4 py-15 lg:py-18">
        <div className="flex justify-center mb-14">
          <div>
            <h2 className="text-[44px] font-semibold leading-[1.08] mt-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {tutors.map((t) => (
            <article className="tutor-card" key={t.name}>
              <div className={`relative w-full aspect-square rounded-2.5xl overflow-hidden mb-4 ${t.image}`}
              >
                <Image src={t.image || DummyTutor} alt={t.name} fill className="object-cover" />
              </div>
              ...
              <div className="flex items-center justify-between mt-auto pt-3.5 border-t border-line">
                ...
                <Link href={`/tutors/${t.id}`} className="book-btn">Book session</Link>
              </div>
            </article>
          ))}
        </div>
