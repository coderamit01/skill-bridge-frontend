    ...
    <section className="section test-bg">
      <div className="max-w-310 mx-auto px-4 py-15 lg:py-18">
        <div className="flex justify-center mb-14">
          <h2 className="text-[44px] leading-[1.08] mt-4 font-semibold">
            ...
            >
              <div className="flex gap-0.5 text-amber">
                {Array.from({ length: review.rating }).map((_, k) => (
                  <StarIcon key={k} width={16} height={16} className="text-yellow-500" />
                ))}
              </div>
              ...
              <div className="flex items-center gap-3 pt-5 border-t border-line">
                <Avatar className="h-10 w-10">
                  <AvatarImage src={review?.student?.image} className="rounded-full" />
                  <AvatarFallback className="bg-[#25a5a21c] text-brand font-bold">{review?.student?.name.charAt(0)}</AvatarFallback>
                </Avatar>
                <div>
                  <div className="font-display text-[15px] font-bold text-ink leading-tight">{review?.student?.name}</div>
                </div>
              </div>
            </div>
          ))}
